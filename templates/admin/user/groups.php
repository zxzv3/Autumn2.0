		...
		.User_group_create .checkbox span{position: relative;top:3px;}
		.User_group_create .limit{border-radius:5px;margin-top:10px;float:left;width:100%;padding:8px 14px;background-color:#f3f3f3;max-height:400px;overflow:hidden;overflow-y:scroll}
		.User_group_create .limit h2{float:left;width:100%;color:#aaa;font-weight:400;font-size:12px;margin-bottom:14px;margin-top:14px}
		.User_group_create .limit .widget-checkbox{margin-top:3px;margin-right:5px}
		.User_group_create .limit label{margin-left:10px;margin-right:15px;margin-bottom:11px;float:left}
...
<body>
	...
	<div class="warpper">
		<div class="tools">
			<button class="btn active" id="js-create-group"><i class="fa fa-user-plus"></i>添加权限组</button>
		</div>
	...
	<script type="text/dom">
		var create_group = <div class="User_group_create" api-name='User_group/Create'>
			<input type="text" placeholder="请输入权限组名称" api-param-name='name' name='权限组名称' min='2' max='16'>
			<div class="limit">
				<?php
					$Jurisdiction = $this->config->item('home_jurisdiction');
		...
		var edit_group = <div class="User_group_create" api-name='User_group/Edit'>
	...
	<script type="text/javascript">
		$(".fa-trash-o").click(function(){
			...
			popup.sure({
				...
				ApiRequest.push('User_group/Remove' , {params : {id : data.id}});
			})
		});
		...
		$(".fa-edit").click(function(){
			...
			popup.sure({
				...
				ApiRequest.push('User_group/Edit' , {params : {
					id : data.id,
					limit : limit
				}})
			})
		...
		$("#js-create-group").click(function(event) {
			popup.sure({
				...
				ApiRequest.push('User_group/Create' , {params : {limit : limit}})
			})
		});
