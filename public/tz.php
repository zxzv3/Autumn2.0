...
?>
	...
	<script language="javascript" type="text/javascript">
		...
		acd2 = new Date();
		acd2ok=acd2.getTime();
		window.location = './public/tz.php?speed=' +(acd2ok-acd1ok)+'#w_networkspeed';
	</script>
<?php
}
//网速测试-结束
...
<!--
.tz * {font-size: 12px;}
.tz body{text-align: center; margin: 0 auto; padding: 0; background-color:#fafafa;font-size:14px;font-family:Tahoma, Arial}
.tz h1 {font-size: 26px; padding: 0; margin: 0; color: #333333; font-family: "Lucida Sans Unicode","Lucida Grande",sans-serif;}
.tz h1 small {font-size: 11px; font-family: Tahoma; font-weight: bold; }
.tz a{color: #666; text-decoration:none;}
.tz a.black{color: #000000; text-decoration:none;}
.tz table{width:100%;clear:both;padding: 0; margin: 0 0 10px;border-collapse:collapse; border-spacing: 0;
}
.tz th{padding: 8px 12px; font-weight:400;background:#f2f2f2;color:#333; text-align:left;}
.tz tr{padding: 0; background:#FFFFFF;}
.tz td{padding: 6px 8px; border:1px solid #eee;}
.tz .w_logo{height:25px;text-align:center;color:#333;FONT-SIZE: 15px; width:13%; }
.tz .w_top{height:25px;text-align:center; width:8.7%;}
.tz .w_top:hover{background:#dadada;}
.tz .w_foot{height:25px;text-align:center; background:#dedede;}
.tz input{padding: 2px; background: #FFFFFF; border-top:1px solid #666666; border-left:1px solid #666666; border-right:1px solid #CCCCCC; border-bottom:1px solid #CCCCCC; font-size:12px}
.tz input.btn{font-weight: bold; height: 20px; line-height: 20px; padding: 0 6px; color:#666666; background: #f2f2f2; border:1px solid #999;font-size:12px}
.tz .bar {border:1px solid #999999; background:#FFFFFF; height:5px; font-size:2px; width:89%; margin:2px 0 5px 0;padding:1px; overflow: hidden;}
.tz .bar_1 {border:1px dotted #999999; background:#FFFFFF; height:5px; font-size:2px; width:89%; margin:2px 0 5px 0;padding:1px; overflow: hidden;}
.tz .barli_red{background:#ff6600; height:5px; margin:0px; padding:0;}
.tz .barli_blue{background:#0099FF; height:5px; margin:0px; padding:0;}
.tz .barli_green{background:#36b52a; height:5px; margin:0px; padding:0;}
.tz .barli_black{background:#333; height:5px; margin:0px; padding:0;}
.tz .barli_1{background:#999999; height:5px; margin:0px; padding:0;}
.tz .barli{background:#36b52a; height:5px; margin:0px; padding:0;}
.tz #page {width: 100%; padding: 0 auto; margin: 0 auto; text-align: left;}
.tz #header{position:relative; padding:5px;}
.tz .w_small{font-family: Courier New;}
.tz .w_number{color: #f800fe;}
.tz .sudu {padding: 0; background:#5dafd1; }
.tz .suduk { margin:0px; padding:0;}
.tz .resYes{}
.tz .resNo{color: #FF0000;}
.tz .word{word-break:break-all;}
-->
</style>

<script language="JavaScript" type="text/javascript" src="./assets/bin/jquery/jquery-1.11.3.min.js"></script>
...

{
	...
	$.getJSON('./public/tz.php?act=rt&callback=?', displayData);
...
<table>
  ...
  <tr>
    ...
    <td colspan="5">
    	<script type="text/javascript">
			$.get('./public/tz.php?cpu_data').then(function(data){
    			$("#js-cpu").html(data)
    		});
