...
class Admin_group extends CI_Controller {
	...
	public function remove(){
		if( ! $this->Admin_group_model->is_exist(array('id' => $this->params->id))){
			Autumn::end(false , '您要删除的权限组不存在');
		}

		$Admin_group_data = $this->Admin_group_model->get(array('id' => $this->params->id));
		if($Admin_group_data['id'] == '1') Autumn::end(false , '您无权编辑该分组');

		$this->load->model('Admin_user_model');
		$this->Admin_user_model->edit(array('from_group' => $Admin_group_data['id']) , array(
			'from_group' => -1
		));

		$this->Admin_group_model->remove(array('id' => $this->params->id));
		Autumn::end(true);
	}
	...
	public function edit(){
		...
		if($this->Admin_group_model->is_exist(array(
			'id' != $this->params->id ,
		...
		$this->Admin_group_model->edit(array('id' => $this->params->id) , array(
			'name' => $this->params->name,
	...
	public function create(){
		$limit = $this->input->post('limit');
		if(count($limit) <= 0) Autumn::end(false , '您输入的允许权限不完整');
		if($this->Admin_group_model->is_exist(array( 'name' => $this->params->name ))){
			Autumn::end(false , '您输入的权限组名称已经存在');
		}
