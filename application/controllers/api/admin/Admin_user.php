...
class Admin_user extends CI_Controller {
	...
	public function remove(){
		...
		if( ! $this->Admin_user_model->is_exist(array('id' => $this->params->id))) Autumn::end(false , '您输入的用户名不存在');
		$User_data = $this->Admin_user_model->get(array('id' => $this->params->id));
		if($User_data['from_group'] == 1) Autumn::end(false , '您无法删除超级管理员帐号');
		$this->Admin_user_model->remove(array('id' => $this->params->id));
		Autumn::end(true);
	}
	...
	public function edit(){
		...
		if( ! $this->Admin_user_model->is_exist(array('id' => $this->params->id))) Autumn::end(false , '您输入的用户名不存在');
		$User_data = $this->Admin_user_model->get(array('id' => $this->params->id));
		if($User_data['from_group'] == 1 && $this->params->group != 1){
			Autumn::end(false , '您无法将超级管理员移动出去');
		}
		...
		if($this->params->group == 1) Autumn::end(false , '超级管理员权限组仅能创建一个帐号');

		$this->Admin_user_model->edit(array('id' => $this->params->id) , array(
	...
	public function create(){
		$this->load->model('Admin_user_model');
		$this->load->model('Admin_group_model');
		if($this->Admin_user_model->is_exist(array('username' => $this->params->username))) Autumn::end(false , '您输入的用户名已经存在');
		if( ! $this->Admin_group_model->is_exist(array('id' => $this->params->group))) Autumn::end(false , '您选择的权限组不存在');
		if($this->params->group == 1) Autumn::end(false , '超级管理员权限组仅能创建一个帐号');
